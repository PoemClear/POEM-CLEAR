<template>
  ...
  >
    <BasicForm @register="registerForm">
      <template #subjectSearch="{ model, field }">
        <ApiSelect
          :api="getSubjectSelectList"
          showSearch
          mode="multiple"
...
<script lang="ts">
  ...
  import { getSubjectSelectList } from '/@/api/content/subject';
  import { Select, Alert } from 'ant-design-vue';
  import { useDebounceFn } from '@vueuse/core';
  import { useMessage } from '/@/hooks/web/useMessage';
  export default defineComponent({
    name: 'RoleDrawer',
    components: { BasicDrawer, BasicForm, ASelect: Select, [Alert.name]: Alert, ApiSelect },
    ...
    setup(_, { emit }) {
      ...
      const searchParams = computed<Recordable>(() => {
        return { title: unref(keyword), pageSize: 100000 };
      });
