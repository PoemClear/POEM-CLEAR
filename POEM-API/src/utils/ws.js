...
//定义变量，用来存储socket。若是是多进程的话，那么socket能够考虑存入redis中
var socketList = {};
//客户端链接服务器socket成功时触发的事件;
io.sockets.on('connection', function (socket) {
    //链接成功时，告诉客户端链接成功;能够经过send来发送信息
    socket.send({status: 1});
    //socket失去链接时触发（包括关闭浏览器，主动断开，掉线等任何断开链接的状况）
...
function broadcast(){
    ...
    // 向another room广播一个事件，在此房间全部客户端都会收到消息
    //注意：这里是从服务器的角度来提交事件
