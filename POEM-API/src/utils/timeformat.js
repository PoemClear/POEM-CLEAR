...
function listMockToTree(data, pid) {
  ...
  data.forEach((item) => {
    ...
    if (!item.children.length || item.children[0].type == 2) {
      delete item.children;
    }
    if (item.type == 0 && item.isShow > 0) {
      if (item.children[0]) {
        ...
        item.meta = {
          ...
          hideChildrenInMenu: item.isShow == 0 ? true : false,
        };
      } else {
        item.path = item.routePath;
        item.name = item.menuName;
    ...
    if (item.type == 1) {
      ...
      item.meta = {
        title: item.name,
        icon: item.icon,
        hideMenu: item.isShow == 0 ? true : false,
        ignoreKeepAlive: item.keepalive == 1 ? true : false,
        orderNo: item.orderNo,
      };
    }
    if (item.type == 2) {
...
function fileSuffixTypeUtil(fileName) {
  // 后缀获取
  var suffix = "";
  // 获取类型结果
  var result = "";
