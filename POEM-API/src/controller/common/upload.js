...
const DB = require("../../db");
const {fileSuffixTypeUtil, rTime, timestamp} = require("../../utils/timeformat");
...
const merge = (HASH, count) => {
    return new Promise( async (resolve, reject) => {
        ...
        console.log( `${uploadDir}/${HASH}.${suffix}----${HASH}.${suffix}`)
 ...
 */
exports.upload_single  = async (req, res) => {
    try {
        ...
        res.send({
            ...
            url: file.path.replace(baseDir, `${process.env.SocketPORT}`),
        });
    } catch (err) {
        res.send({
            code: 1,
 ...
 * @returns {Promise<void>}
 */
exports.upload_single_base64= async (req, res) => {
    ...
    if (isExists) {
        res.send({
            ...
            urlname: filename,
            url: path.replace(baseDir, `${process.env.SocketPORT}`),
        });
        return;
    }
    // fs.writeFile(res)
...
//处理缩略图接口
exports.upload_single_name=async (req, res) => {
    try {
 ...
 * 上传切片
 */
exports.upload_chunk =  async (req, res) => {
    try {
        ...
        if (isExists) {
            res.send({
                ...
                url: path.replace(`${process.env.SocketPORT}`, 123),
            });
            return;
        }
 ...
 * 合并切片
 */
exports.upload_merge= async (req, res) => {
...
exports.upload_already= async (req, res) => {
    let { HASH } = req.body
    let path = `${uploadDir}/${HASH}`
    let fileList = []
    try {
