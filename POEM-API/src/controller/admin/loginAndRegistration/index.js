...
const { rTime, timestamp, listMockToTree } = require("../../../utils/timeformat")
const DB = require("../../../db")
...
exports.login = async (req, res) => {
    ...
    let userInfo = await DB(res, 'sy_users', 'find', '服务器错误', `username='${username}'  or account='${account}' `)
    // let roleInfo = await DB(res, 'sy_roles', 'find', '服务器错误', )
    if (!userInfo[0]) {
        return res.json({
            code: 403, message: "账号不存在"
        })
    }
    if (md5(md5(password) + config.md5Str) != userInfo[0].password) {
        res.json({ code: 403, message: "密码输入有误" });
        return
    }
    if (userInfo[0].status == 0) {
        res.json({
            success: false, code: 403, message: "账号未激活"
        })
        return
    }
    ...
    await DB(res, 'sy_users', 'update', '服务器错误', `username='${username}'  or account='${username}' `, {
        lastLoginTime: (timestamp(new Date()))
    })
    res.json({
        code: 200, message: "登录成功", result: {
            token,
...
exports.userInfo = async (req, res) => {
    ...
    res.json({
        code: 200, type: "success", message: "ok", result:
            userInfo[0]
...
exports.getPermCode = async (req, res) => {
    ...
    let result = await DB(res, 'sy_menus', 'find', '服务器出错', `status='0'  order by orderNo desc`);
