...
let orm_config = {host: config.mysql.hp.host,port:config.mysql.port,user: config.mysql.user,password: config.mysql.hp.password,database: config.mysql.database}
...
let orm = {
    ...
    model: function (name, options) {
        ...
        str = str.replace(/,$/, '');
        tableSQL = `CREATE TABLE if not EXISTS ${name} (${str})`;
