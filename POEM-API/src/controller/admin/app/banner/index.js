...
exports.multipleDelBanner = async (req, res) => {
  ...
  let ret = await DB(
    ...
    "sql",
    "服务器错误",
    `select *  from xcx_banner delete where id in ${ids}`
  );
  if (ret.affectedRows == 1) {
    res.json({
      code: 200,
...
exports.bannerList = async (req, res) => {
  ...
  let list = await DB(res, "sy_dict", "find", "服务器出错", `value='bannerPosition' and parentId=0`);
  let cateList = []
  if (list[0]) {
  ...
  let bannerLen = await DB(
    ...
    "find",
    "服务器出错",
    `remark like '%${params.remark}%' and status like '%${params.status}%' and type like '%${params.type}%' `
  );
  let result = await DB(
    res,
    "xcx_banner",
    "find",
    "服务器出错",
    `remark like '%${params.remark}%' and status like '%${
      params.status
