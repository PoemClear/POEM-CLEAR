...
exports.getMenuList = async (req, res) => {
  ...
  let result = await DB(
    ...
    "find",
    "服务器出错",
    `status like '%${params.status}%' and menuName like '%${params.menuName}%' order by orderNo desc`
  );
  result.forEach((v) => {
    v.id = v.id + "";
    if (v.createTime) {
...
exports.getAsyncMenuList = async (req, res) => {
  ...
  let parentIdsAndPermissionMenuIds = uniqueNumber([
    ...permissionMenuIds,
    ...parentIds,
  ]);

  let list = result.filter((item) =>
    parentIdsAndPermissionMenuIds.includes(item.id)
  );
  list.forEach((v) => {
    if (v.type !== 2) {
      v.id = v.id + "";
